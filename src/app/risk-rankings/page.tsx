...
const RiskRankins: React.FC = () => {
  ...
  const handleAddRow = () => {
    ...
    setriskRankings([...riskRankings, newData]);
    const dataApa = dataGlobal;
    dataApa.Risk_Criteria.Risk_Rankings = riskRankings;
  ...
  const handleRemoveActiveRow = () => {
    if (activeRow !== null) {
      const updatedData = [...riskRankings];
      updatedData.splice(activeRow, 1);
      setriskRankings(updatedData);
      const dataApa = dataGlobal;
      dataApa.Risk_Criteria.Risk_Rankings = riskRankings;
      updateDataGlobal(dataApa);
      setActiveRow(null);
  ...
  const moveDown = () => {
    if (activeRow !== null) {
      if (activeRow < riskRankings.length - 1) {
        const updatedData = [...riskRankings];
        const temp = updatedData[activeRow];
        updatedData[activeRow] = updatedData[activeRow + 1];
        updatedData[activeRow + 1] = temp;
  ...
  const handlePriorityChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    const updatedData = [...riskRankings];
    updatedData[index].Priority = e.target.value;
    setriskRankings(updatedData);
    const dataApa = dataGlobal;
    dataApa.Risk_Criteria.Risk_Rankings = riskRankings;
    updateDataGlobal(dataApa);
  };
  const handleLOPAChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    const updatedData = [...riskRankings];
    updatedData[index].Required_Lopa_Credits = e.target.value;
    setriskRankings(updatedData);
    const dataApa = dataGlobal;
    dataApa.Risk_Criteria.Risk_Rankings = riskRankings;
    updateDataGlobal(dataApa);
  ...
  return (
    <>
      <PageHeader title="Risk Rankings" />
          ...
          <div className="row">
            <div className="w-1/6">
              <ul>
                ...
                <li className="mb-4 lg:mb-2"><Link href="/risk-rankings" className="block hover:text-gray-900 text-lg text-black">Risk Ranking</Link></li>
              </ul>
            </div>
            <div className="w-5/6">
              <h1>Risk Rankings</h1>
              ...
              <table className='table-auto'>
                <thead className='bg-slate-300'>
                  <tr>
                    ...
                    <td className="border px-4 py-2">Required LOPA Credits</td>
                  </tr>
                </thead>
                <tbody>
